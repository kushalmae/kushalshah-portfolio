import PageLayout from "@/components/PageLayout";
import SectionLabel from "@/components/SectionLabel";
import Reveal from "@/components/Reveal";
import Seo from "@/components/Seo";
import ContactForm from "@/components/ContactForm";
import { site } from "@/config/site";

const Contact = () => (
  <PageLayout title="Contact">
    <Seo
      title="Contact"
      description="Open to technical program leadership, systems engineering management, and select advisory work. LinkedIn is the fastest channel."
      path="/contact"
    />
    <section className="py-24 md:py-40">
      <div className="container max-w-3xl">
        <Reveal>
          <SectionLabel>Contact</SectionLabel>
          <h1 className="text-3xl md:text-4xl font-semibold tracking-tight text-foreground mb-6">
            Let's build something mission-critical.
          </h1>
          <p className="text-muted-foreground leading-relaxed mb-12 max-w-xl">
            I am open to technical program leadership, systems engineering management, and select advisory engagements where cross-functional execution and technical rigor drive measurable outcomes.
          </p>
        </Reveal>

        {/* LinkedIn first — this is the channel recruiters and hiring managers actually use. */}
        <div className="space-y-6 mb-16">
          <Reveal>
            <div>
              <h3 className="font-mono text-xs tracking-[0.2em] uppercase text-muted-foreground mb-2">LinkedIn</h3>
              <a
                href={site.linkedin}
                target="_blank"
                rel="noopener noreferrer"
                className="text-foreground hover:text-primary transition-colors text-base"
              >
                linkedin.com/in/kushalmae
              </a>
            </div>
          </Reveal>
          <Reveal delay={60}>
            <div>
              <h3 className="font-mono text-xs tracking-[0.2em] uppercase text-muted-foreground mb-2">Email</h3>
              <a href={`mailto:${site.email}`} className="text-foreground hover:text-primary transition-colors text-base">
                {site.email}
              </a>
            </div>
          </Reveal>
        </div>

        <div className="h-px bg-line mb-12" />

        <Reveal>
          <p className="font-mono text-xs tracking-[0.2em] uppercase text-muted-foreground mb-6">
            Or send a structured message
          </p>
        </Reveal>

        <Reveal delay={80}>
          <ContactForm />
        </Reveal>

        <div className="h-px bg-line my-12" />

        <Reveal>
          <p className="text-sm text-muted-foreground">
            Based in Cypress, California. Open to remote and hybrid engagements.
          </p>
        </Reveal>
      </div>
    </section>
  </PageLayout>
);

export default Contact;
